/*
[_object,_type] spawn server_updatObiect;
*/
...
_object_position = {
	...
	if (_object isKindOf "AllVehicles") then {
		//diag_log("_object_position AllVehicles");
		_fuel = fuel _object;
...
_object_inventory = {
	if (_naObnovku) then {
		...
		//diag_log("_object_inventory");
		if (str(_inventory) != _previous) then {
			_object setVariable["lastInventory",_inventory];
			//diag_log("_object_inventory NEW");
			if (_objectID == "0") then {
...
_object_damage = {
	...
	if (!(typeOf _object in _allowed)) then {
		...
		//diag_log format ["do _array = %1",_array];
		...
		for "_mti" from 0 to _tc-1 do {
			...
			if (_st in ["front_plow","wheel_guards","window_guards","windshield_guard"]) then {
				...
				//diag_log format ["v seredine _st=%2,  _array = %1",_array,_st];
			};
		};
		//diag_log format ["posle _array = %1",_array];
