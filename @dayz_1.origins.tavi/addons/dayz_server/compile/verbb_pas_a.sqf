private ["_object","_objectLevel","_playerID","_passUch","_fill_house","_objIdKey","_type","_intentory","_result","_result2","_plusnum","_dobav","_dobavL","_dobavR","_key","_data","_key2","_data2","_strArr","_strCount","_giveMeMaterial","_key3","_result3","_objWpnTypes","_objWpnQty","_countr","_isOK","_block","_howmany","_stopGo","_objectC","_playerC","_objectLevelC","_playerIDC","_passwordC","_flagTo","_RealPass","_playerIDS","_objectLevelS","_objectID","_object_position","_isNotOk","_naObnovku","_inventoryEnd"];
//verbb_pas = [_object,player,_objectLevel,_playerID,_passUch select 0];
...
_fill_house = {
	...
	_dobavL = false;
	_dobavR = false;
	_giveMeMaterial = false;
		
	_key = format["CHILD:501:object.object_data:[""Inventory""]:[[""ObjectID"",""="",""%1""]]:[0,1]:", _objIdKey];
	_result = [_key] call server_hiveReadWrite;
	//_data = "HiveEXT" callExtension _key;
	//_result = call compile format ["%1",_data];
	diag_log format ["_result = %1, %2",_result, typeName _result];
	
	//_result = ["PASS",""82855e71""];
	_key2 = format["CHILD:504:'%1':",_result select 1];
	diag_log format ["_key2 = %1, %2",_key2, typeName _key2];
	...
	//_result2 = call compile format ["%1",_data2];
	diag_log format ["_result2 = %1, %2",_result2, typeName _result2];
	if(_result2 select 0 == "PASS") then {
		_intentory = ((_result2 select 1) select 0);
		_giveMeMaterial = _object getVariable ["givemeMaterial",false];
		if (_giveMeMaterial) then {
			_dobav = true;
			_dobavL = true;
			_dobavR = true;
			_object setVariable ["givemeMaterial",false,false];
		};
	...
	diag_log format ["CHILD 505 _result3 = %1, inv = %2",_result3, count _intentory];
...
_RealPass = 929909929;
_objIdKey = "netego";
//_this=[20daeb00# 782108: bandits_log_house_lvl_2.p3d,B 1-1-B:1 (Indiango) REMOTE,3,"3231556",147258]
