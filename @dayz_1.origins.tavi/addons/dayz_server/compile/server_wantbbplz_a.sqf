...
if (_typN2 == "STRING") then {
	...
	if (!_stopGo) then {
		//_hasBPitem = _item in magazines _player;
		//if (!_hasBPitem) exitWith { diag_log format ["ERROR_BB _item(%3) not in inventory of _playerID(%1) recived from %2",_playerID,_player]; };
		...
		if (_item == "ItemBpt_b1") then { 
			if (_playerID in bb_base_level1b) exitWith { diag_log format ["ERROR_BB _playerID(%1) already have level 1b(%2)",_playerID,_item]; };
				if (_playerID == "20016198") then { 
					_housetype = "object_x"; _levelhouse = 1;
					bb_base_level1b set [count bb_base_level1b, _playerID]; _canBuildH = true;
					_stageAr = [["stage_1",0],[_pNameR,9999999],["passwordtut",4835],["flag",1]];
				} else {
